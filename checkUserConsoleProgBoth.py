...
#####################################
Rule Name:
  iam-user-console-prog-both-check
Description:
  IAM USER가 콘솔/프로그래매틱 권한을 모두 가지고 있는지 확인
Trigger:
  Periodic
...
Rule Parameters:
  ...
  | ------------------- | --------- | -------------------------------------------------------- |
  | WhitelistedUserList | Optional  |  두 권한 모두 있어도 되는 화이트리스트                         /
  | ------------------- | --------- | -------------------------------------------------------- |
...
Scenarios:
  ...
  Given: WhitelistedUserList is configured correctly
    ...
    And: programmatic 권한은 없으면
    Then: Return COMPLIANT
  Scenario 4:
  Given: WhitelistedUserList is configured correctly
    And: The IAM User UniqueID is not listed in WhitelistedUserList
    And: 콘솔/Programmatic 권한이 모두 있으면
    Then: Return NON_COMPLIANT
  Scenario 5:
  Given: WhitelistedUserList is not configured
    And: 콘솔/programmatic 계정 중 하나만 가지고 있으면
   Then: Return COMPLIANT
  Scenario 6:
  Given: WhitelistedUserList is not configured
    And: 콘솔/Programmatic 권한이 모두 있으면
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    logging.info(all_users_list)
    evaluations = []
    for user in all_users_list:
        if user['UserId'] in valid_rule_parameters:
            evaluations.append(build_evaluation(user['UserId'], 'COMPLIANT', event,
                                                annotation='The user ({}) is whitelisted.'.format(user['UserName'])))
            continue

        try:
            iam_client.get_login_profile(UserName=user['UserName'])
            # console 계정 있는 user
            check_programmatic(iam_client, event, evaluations, user, 1)

        except iam_client.exceptions.NoSuchEntityException:
            # console 계정 없는 user
            check_programmatic(iam_client, event, evaluations, user, 2)

    return evaluations

def check_programmatic(iam_client, event, evaluations, user, flag):
    #flag 1 = console 계정 있음
    #flag 2 = console 계정 없음
    msg=''

    accesskey = iam_client.list_access_keys(UserName=user['UserName'])['AccessKeyMetadata']
    if not accesskey:
        if flag == 2:
            msg = 'Username({}) - 콘솔/프로그래매틱 권한 모두 미존재'.format(user['UserName'])
            evaluations.append(build_evaluation(user['UserId'], 'NON_COMPLIANT', event, annotation=msg))
            logger.info(msg)
        else:
            #console 권한만 존재
            evaluations.append(build_evaluation(user['UserId'], 'COMPLIANT', event))
            logger.info(user['UserName']+' console 권한만 존재')
    else:
        if flag == 1:
            msg = 'Username({}) - 콘솔/프로그래매틱 권한 모두 존재'.format(user['UserName'])
            evaluations.append(build_evaluation(user['UserId'], 'NON_COMPLIANT', event,annotation=msg))
            logger.info(msg)
        else:
            # programmatic 권한만 존재
            evaluations.append(build_evaluation(user['UserId'], 'COMPLIANT', event))
            logger.info(user['UserName']+ 'programmatic 권한만 존재')
...
def lambda_handler(event, context):
    #(CreateLoginProfile) Give the user Console access. This requires a password. You must also give the user the URL of your account's sign-in page.
    #(CreateAccessKey) Give the user programmatic access. This requires access keys.
