...
def evaluate_compliance(event, context, configuration_item, valid_rule_parameters):
    ...
    try:
        ...
        return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                annotation="Unable to load most recent template from S3. Auto-deployment has been triggered.")
    ...
    try:
        ...
        return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                annotation="Unable to get status of Config Rules.")
    ...
    for k, resource in template["Resources"].items():
        if resource["Type"] == "AWS::Config::ConfigRule":
                ...
                if rule["ConfigRuleName"] == resource["Properties"]["ConfigRuleName"]:
                    ...
                    if "Scope" in resource["Properties"]:
                        if "Scope" not in rule:
                            return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                                    annotation="The rule (" + rule[
                                                        "ConfigRuleName"] + ") has an incorrect 'Scope' configuration.")
                    ...
                    if "Source" in resource["Properties"]:
                        if "Source" not in rule:
                            return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                                    annotation="The rule (" + rule[
                                                        "ConfigRuleName"] + ") has an incorrect 'Source' configuration.")
                        if resource["Properties"]["Source"]["Owner"] != rule["Source"]["Owner"]:
                            return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                                    annotation="The rule (" + rule[
                                                        "ConfigRuleName"] + ") has an incorrect 'Owner' configuration.")
                        ...
                        if 'Fn::Sub' in resource["Properties"]["Source"]['SourceIdentifier']:
                            resource_lambda = resource["Properties"]["Source"]['SourceIdentifier']['Fn::Sub'].replace(
                                '${AWS::Partition}', compliance_account_partition).replace('${AWS::Region}',
                                                                                           compliance_account_region).replace(
                                '${LambdaAccountId}', compliance_account_id)
                        else:
                        ...
                        if resource_lambda != rule["Source"]["SourceIdentifier"]:
                            return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                                    annotation="The rule (" + rule[
                                                        "ConfigRuleName"] + ") has an incorrect 'SourceIdentifier' configuration.")
                    ...
                    if rule["ConfigRuleState"] != "ACTIVE":
                        return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                                annotation="The rule (" + rule["ConfigRuleName"] + ") is not active.")
            ...
            if not rule_found:
                ...
                return build_evaluation(invoking_account_id, "NON_COMPLIANT", event,
                                        annotation="The rule (" + resource["Properties"][
                                            "ConfigRuleName"] + ") is not deployed.")
...
def get_all_compliance_evaluations(rule_name):
    ...
    while True:
        ...
        if 'NextToken' in all_eval_part:
            ...
            all_eval_part = AWS_CONFIG_CLIENT.get_compliance_details_by_config_rule(ConfigRuleName=rule_name,
                                                                                    NextToken=next_token, Limit=100)
        else:
            break
    return all_eval
